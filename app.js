...
const saveDataToLocalStorage = () => {
  ...
  const data = rows.map((row) => {
    const name = row.querySelector(".nameTd").textContent;
    const surname = row.querySelector(".surnameTd").textContent;
    const scores = row.querySelector(".scoreTd").textContent;
    return {
      name,
      surname,
      scores,
...
const loadDataFromLocalStorage = () => {
  ...
  if (data) {
    data.forEach((item) => {
      ...
      const scoreTd = document.createElement("td");
      scoreTd.classList.add("scoreTd");
      scoreTd.textContent = item.scores;
